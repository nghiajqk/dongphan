...
local rangeBox = createInput("Tăng tầm đánh", 15, 310)
...
-- Di chuyển quanh boss và tạo hitbox mở rộng
	...
	while true do
		...
		if running then
			...
			if target and target:FindFirstChild("HumanoidRootPart") then
				...
				-- Tạo hoặc cập nhật hitbox mở rộng
				local tool = char:FindFirstChildOfClass("Tool")
				if tool and tool:FindFirstChild("Handle") then
					local handle = tool.Handle
					local existing = tool:FindFirstChild("ExtendedHitbox")
					if not existing then
						...
						hitbox.Size = Vector3.new(range, 5, range)
						hitbox.Parent = tool
						local weld = Instance.new("WeldConstraint")
						weld.Part0 = handle
						weld.Part1 = hitbox
						weld.Parent = hitbox
						hitbox.CFrame = handle.CFrame * CFrame.new(0, 0, -range / 3)
					else
						existing.Size = Vector3.new(range, 5, range)
						existing.CFrame = handle.CFrame * CFrame.new(0, 0, -range / 3)
					end
				end
			end
		end
