...
task.spawn(function()
	...
	-- Tăng tầm đánh bằng cách tạo hitbox ảo
task.spawn(function()
    wait(1) -- chờ nhân vật load hoàn chỉnh
    local tool = nil

    -- Tìm Tool (vũ khí) trong nhân vật
    ...
    if tool and tool:FindFirstChild("Handle") then
        ...
        -- Kiểm tra nếu chưa có Hitbox thì mới thêm
        if not tool:FindFirstChild("ExtendedHitbox") then
            local hitbox = Instance.new("Part")
            hitbox.Name = "ExtendedHitbox"
            hitbox.Size = Vector3.new(15, 5, 15) -- Tăng size ở đây để mở rộng hitbox
            hitbox.Transparency = 1 -- vô hình
            hitbox.CanCollide = false
            hitbox.Anchored = false
            hitbox.Massless = true
            hitbox.Parent = tool

            -- Gắn cố định vào handle (vũ khí)
            local weld = Instance.new("WeldConstraint")
            weld.Part0 = handle
            weld.Part1 = hitbox
            weld.Parent = hitbox

            -- Đặt hitbox phía trước vũ khí
            hitbox.CFrame = handle.CFrame * CFrame.new(0, 0, -6)

            print("✅ Đã tạo hitbox mở rộng tầm đánh")
        end
    else
        warn("⚠️ Không tìm thấy Tool hoặc Handle để tạo hitbox")
    end
end)
