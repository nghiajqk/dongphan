...
-- Auto chạy vòng (sửa lỗi bị tele lên trời hoặc dưới map)
task.spawn(function()
	while true do
		wait()
		if running then
			...
			if target then
				...
				local desiredPos = tpos + offset + Vector3.new(0, 10, 0) -- đặt raycast trên cao hơn mặt đất

				-- Raycast tìm mặt đất bên dưới điểm desiredPos
				...
				if rayResult then
					local safePos = rayResult.Position + Vector3.new(0, 2, 0) -- đứng cao hơn mặt đất 2 đơn vị
					char:MoveTo(safePos)
				end
			end
		end
